...
class optics_tmm(comp_utils):
    def __init__(self, device=None):
        ...
        self.kx = 0.0
        self.log_cap = -400
        self.zm = torch.zeros((2, 2), dtype=torch.complex64, device=self.device)
        self.a0 = 1
        self.bN = torch.zeros(1, dtype=torch.complex64, device=self.device)
        return
        ...
        if 'third_vars' in sim_params:
            if isinstance(sim_params['third_vars'], list):
                if len(sim_params['third_vars']) > 0:
                    kx = torch.tensor(np.sin(sim_params['third_vars'][0].get('incidentAngle', 0.0)),
                                    device=self.device)
            else:
                kx = torch.tensor(np.sin(sim_params['third_vars'].get('incidentAngle', 0.0)),
                                device=self.device)
        self.kx = kx
        return {'kx': self.kx}
    ...
    def mul(self, a, b):
        ...
        if torch.is_tensor(a) and torch.is_tensor(b):
            return torch.matmul(a.to(self.device), b.to(self.device))
        elif isinstance(a, np.ndarray) and isinstance(b, np.ndarray):
            return super().mul(a, b)
        else:
            # Convert to PyTorch tensors if mixed types
            a_tensor = torch.tensor(a, dtype=torch.complex64, device=self.device) if not torch.is_tensor(a) else a.to(self.device)
            b_tensor = torch.tensor(b, dtype=torch.complex64, device=self.device) if not torch.is_tensor(b) else b.to(self.device)
    ...
    def interface_tm(self, x, y, yp1, sim_params, tracked_info, mat1Left={}, mat2Left={}, mat1Right={}, mat2Right={}):
        ...
        if not self.n_is_optParam:
            if self.log_ratio:
                ...
                y = torch.tensor((100**y_val - 1)/99, dtype=torch.float32, device=self.device)
                yp1 = torch.tensor((100**yp1_val - 1)/99, dtype=torch.float32, device=self.device)
            else:
                y = torch.tensor(self.safe_extract_value(y), dtype=torch.float32, device=self.device)
                yp1 = torch.tensor(self.safe_extract_value(yp1), dtype=torch.float32, device=self.device)

            # Get refractive indices and interpolate
            n1L = torch.tensor(self.safe_extract_value(mat1Left), dtype=torch.complex64, device=self.device)
            n2L = torch.tensor(self.safe_extract_value(mat2Left), dtype=torch.complex64, device=self.device)
            n1R = torch.tensor(self.safe_extract_value(mat1Right), dtype=torch.complex64, device=self.device)
            n2R = torch.tensor(self.safe_extract_value(mat2Right), dtype=torch.complex64, device=self.device)

            # Interpolate refractive indices
            nL = y * n2L + (1-y) * n1L
            nR = yp1 * n2R + (1-yp1) * n1R
        else:
            nL = torch.tensor(self.safe_extract_value(y), dtype=torch.complex64, device=self.device)
            nR = torch.tensor(self.safe_extract_value(yp1), dtype=torch.complex64, device=self.device)
    ...
    def tm(self, x, xp1, y, yp1, sim_params, tracked_info, mat1params={}, mat2params={}):
        if tracked_info is None or 'kx' not in tracked_info:
            ...
            tracked_info['kx'] = torch.sin(torch.tensor(incident_angle, dtype=torch.float64, device=self.device))
    ...
    def d_interface_tm(self, x, y, yp1, sim_params, tracked_info, tm, mat1Left={}, mat2Left={}, mat1Right={}, mat2Right={}):
        ...
        if not self.n_is_optParam:
            ...
            k0 = 2 * torch.pi / torch.tensor(float(sim_params['simPoint']), dtype=torch.complex64, device=self.device)
            kL = k0 * (y * n2L + (1-y) * n1L)
    ...
    def costFunc_navgtv(self, simPoints, callPoints, third_vars, cur_x, cur_y, all_mat1params,
        ...
        L = torch.tensor(0.0, dtype=torch.float64, device=self.device)
    ...
    def d_costFunc_fields(self, simPoints, callPoints, third_vars, cur_x, cur_y, all_mat1params,
        ...
        for n, (fields, cf_factors) in enumerate(zip(all_fields, all_cf_factors)):
            ...
            dT = torch.stack([self.dTde(torch.tensor(e, dtype=torch.complex64, device=self.device)) for e in fields])
